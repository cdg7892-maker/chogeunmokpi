...
export function generateStaticParams() {
  return womenDetailPages.map((page) => ({ slug: page.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const page = getWomenDetailPage(slug);
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `/programs/women/${page.slug}`,
    },
    ...programImageMetadata({
      title,
      description,
      url: `/programs/women/${page.slug}`,
...
export default async function WomenConditionPage({ params }: PageProps) {
  const { slug } = await params;
  const page = getWomenDetailPage(slug);
    ...
    <main className="min-h-screen bg-[#f6f2e8] text-[#1f241b]">
        ...
        <div className="mx-auto flex max-w-7xl flex-col gap-14 px-5 py-12 sm:px-8 lg:grid lg:grid-cols-[0.95fr_1.05fr] lg:items-center lg:gap-16 lg:py-20">
          <div>
            <nav className="mb-10 text-sm font-medium text-[#7c6f5a]">
              ...
              <Link href="/programs/women" className="hover:text-[#263020]">
                여성질환 클리닉
              </Link>
              <span className="mx-2">›</span>
      ...
      <section className="mx-auto max-w-7xl px-5 py-20 sm:px-8 lg:py-24">
        <SectionHeading
          ...
          desc="여성질환은 수치와 검사만으로 설명되지 않는 생활 신호가 많습니다. 비슷한 표현이 있다면 몸의 흐름을 함께 봐야 합니다."
        />
      ...
      <section className="border-y border-[#d8cfbd] bg-[#fffdf7]">
        <div className="mx-auto max-w-7xl px-5 py-20 sm:px-8 lg:py-24">
          <SectionHeading
            ...
            desc="자궁과 월경의 문제는 하복부 온도, 혈류, 소화, 수면, 감정 리듬과 연결되어 나타납니다."
          />
          <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      ...
      <section className="mx-auto max-w-7xl px-5 py-20 sm:px-8 lg:py-24">
        <SectionHeading
          ...
          desc="증상만 누르면 다시 반복됩니다. 냉열, 어혈, 습담, 기혈의 어느 축이 흔들리는지 봐야 합니다."
        />
      ...
      <section className="border-y border-[#d8cfbd] bg-white">
        <div className="mx-auto max-w-7xl px-5 py-20 sm:px-8 lg:py-24">
          <SectionHeading
            ...
            title="유형별 맞춤 솔루션"
            desc="같은 여성질환이라도 통증의 양상, 냉열, 혈색, 피로도에 따라 치료 방향은 달라집니다."
          />
      ...
      <section className="mx-auto max-w-7xl px-5 py-20 sm:px-8 lg:py-24">
        <SectionHeading
          ...
          desc="여성질환은 주기와 함께 변화합니다. 한 번의 증상보다 여러 주기의 흐름을 관찰합니다."
        />
        <div className="mt-10 grid gap-5 lg:grid-cols-3">
      ...
      <section className="border-y border-[#d8cfbd] bg-[#fffdf7]">
        <div className="mx-auto max-w-7xl px-5 py-20 sm:px-8 lg:py-24">
          <SectionHeading
            ...
            title="초근목피 여성질환 치료 프로그램"
            desc="증상 완화와 주기 회복, 재발 방지를 함께 설계합니다."
          />
        ...
        <div className="mx-auto grid max-w-7xl gap-12 px-5 py-20 sm:px-8 lg:grid-cols-[0.8fr_1.2fr] lg:py-24">
          <aside className="rounded-[24px] border border-[#d8cfbd] bg-white/78 p-8">
            ...
            <p className="mt-6 text-xl font-semibold leading-9 text-[#2c3526] keep-words">
              여성질환은 참는 시간이 길어질수록 몸의 리듬이 더 깊게 흔들릴 수 있습니다. 주기와 생활 신호를 함께 읽겠습니다.
            </p>
        ...
        <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[1fr_0.65fr] lg:items-center">
          <div>
            ...
            <h2 className="mt-4 font-serif text-4xl font-semibold leading-tight sm:text-5xl keep-words">
              반복되는 {page.title}, 몸의 리듬부터 다시 살펴보세요
            </h2>
            <p className="mt-5 max-w-3xl text-lg leading-8 text-[#f2ead8] keep-words">
              검사상 큰 이상이 없더라도 불편함이 반복된다면 자궁 순환과 전신 컨디션의 균형을 함께 확인할 때입니다.
            </p>
          </div>
      ...
      <section className="bg-[#f6f2e8] px-5 py-10 sm:px-8">
        <div className="mx-auto max-w-7xl rounded-2xl border border-[#d8cfbd] bg-white/65 p-6 text-sm leading-7 text-[#5b5242] keep-words">
          ...
          <br />
          본 콘텐츠는 일반적인 한방 의료 정보 제공을 위한 참고 자료이며, 개별 환자의 진단·처방을 대신할 수 없습니다. 심한 출혈, 급성 골반통, 임신 가능성, 고열이나 악취가 동반되는 분비물 등은 즉시 산부인과 진료가 필요할 수 있습니다.
        </div>
      </section>
    </main>
  );
